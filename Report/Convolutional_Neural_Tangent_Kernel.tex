...
%	the first one being a simple Multi-Layer Perceptron (MLP) and the second one being a Convolutional NN.
	...
	In \cite{jacot2018neural}, the authors show that an MLP in the IWL trained using gradient descent evolves, during training time $t$, along the (negative) gradient kernel
	
	This process has a kernel and, during training, the gradient follows a direction given by the \emph{Neural Tangent Kernel} (NTK), which is defined as
	\begin{equation}
		[\NTK(f)]_{i,j} = \left\langle \frac{\partial f (\theta, x_i)}{\partial \theta}, \frac{\partial f (\theta, x_j)}{\partial \theta} \right\rangle.
	\end{equation}
	
	
%	\section{Asymptotics results}\label{sec:asympthotics}
	
	\section{Exact computation}\label{sec:computation}
